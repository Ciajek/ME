...
umnk3 = lm(death.rate~ lhealth.expenditure +gini.index+life.expectancy+employment.industry+air.pollution, data = DATA, weights = DATA$weights3)
...
umnk4 = lm(death.rate~ lhealth.expenditure + lgini.index + life.expectancy+employment.industry+lair.pollution, data = DATA, weights = DATA$weights4)


summary(umnk4)

stargazer(model4,umnk4, type = "text")
